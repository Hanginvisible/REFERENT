using CVAN.GW.WServiceMQDB.Properties;
...
namespace CVAN.GW.WServiceMQDB
{
        ...
        static void Main(string[] args)
        {
            ...
            using (var channel = connection.CreateModel())
            {
                ...
                consumer.Received += (model, ea) =>
                {
                    ...
                    {
                        //Convert from DecodeBase64
                        //message = CVAN.GW.Core.Utility.DecodeBase64(System.Text.Encoding.UTF8, message);
                        //if (string.IsNullOrEmpty(message))
                        //{
                        //    Console.WriteLine(" [->] Sender {Convert from base64 error}");
                        //}
                        //else
                        //{
                        //    Console.WriteLine(" [->] Sender {Convert from base64 oke}");
                        //}
                        //end
                        Console.WriteLine("--------------------------------------------------");
                        var result = "1";
                        ...
                        {
                            string[] arrMessage = Regex.Split(message, "{#####}");
                            if (arrMessage.Length > 0)
                            {
                                Console.WriteLine(" [->] Received {" + DateTime.Now.ToString("dd/MM/yyyy HH-mm-ss") + "}");
                                Console.WriteLine(" [->] Sender {" + arrMessage[0] + "}");
                                Console.WriteLine(" [->] Message {" + arrMessage[1] + "}");
                                Console.WriteLine(" [->] Ivan code {" + arrMessage[2] + "}");
                            }
                        }
                        else
                        {
                    ...
                    //CVAN.GW.DataObjects.DataObjectFactory.GetInstanceCVAN_MSGO().CVAN_MSGI_INSERT_MSGXML_2_MSGOUT(ea.Exchange, ea.Body, ConfigurationManager.AppSettings["MQ_NAME"]);
                ...
                //System.Resources.ResourceManager rm = CVAN.GW.WServiceMQDB.Properties.Resources.ResourceManager;
