...
namespace CMCLIS.CVAN.CORE.Provider
{
    ...
    {        
        ...
        {
            CMCLIS.CVAN.CORE.NetworkShare.DisconnectFromShare(Config.NETWORK_SHARE_DIRECTORY, true); //Disconnect in case we are currently connected with our credentials;
            CMCLIS.CVAN.CORE.NetworkShare.ConnectToShare(Config.NETWORK_SHARE_DIRECTORY, Config.NETWORK_SHARE_ACCOUNT, Config.NETWORK_SHARE_PASSWORD); //Connect with the new credentials 
            DataSet dataSet = GetDataSet(TSQL, maSoThue, maTinhThanh, null);
            CMCLIS.CVAN.CORE.NetworkShare.DisconnectFromShare(Config.NETWORK_SHARE_DIRECTORY, false); //Disconnect from the server.    
            return dataSet;
        }
