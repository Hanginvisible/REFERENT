...
namespace CMCLIS.GATEWAY.DATA.OBJECTS
{
    ...
    public class CVAN_DM_LT_THUEDao : OracleBaseImpl<CVAN_DM_LT_THUEInfo>
    {
        ...
        public List<CVAN_DM_LT_THUEInfo> CVAN_DM_LT_THUE_GetAllWithPadding(string _CVAN_CODE,string _CVAN_NAME,string _CVAN_PARENT,int _CVAN_STATUS,int _IS_DELETE,int pageIndex, int pageSize,ref int totalRecord)
        {
		 ...
		 public List<CVAN_DM_LT_THUEInfo> CVAN_DM_LT_THUE_GetListByLucene(string keyword, int selectTop)
        {
            try
            {
                string[] fieldSelect = {"ID","CVAN_CODE","CVAN_NAME","CVAN_PARENT","CVAN_PARENT_NAME","CVAN_DESCRIPTION","CVAN_STATUS","IS_DELETE","CDATE","LDATE","CUSER","LUSER"};
		...
		#region Using Solr
		 ...
		 public List<CVAN_DM_LT_THUEInfo> CVAN_DM_LT_THUE_QuerySolr_GetAllWithPadding(string keyword, string _CVAN_CODE,string _CVAN_NAME,string _CVAN_PARENT,int _CVAN_STATUS,int _IS_DELETE, int pageIndex, int pageSize,ref int totalRecord)
		 {
		     try
		     {
		         List<ISolrQuery> lstFilter = new List<ISolrQuery>();
		         ...
		         if (_CVAN_STATUS != -1)
		         {
		            if(_CVAN_STATUS == 1)
		            {
		                lstFilter.Add(new SolrQuery("CVAN_STATUS:true"));
		            }
		            else if (_CVAN_STATUS == 0)
		            {
		                lstFilter.Add(new SolrQuery("CVAN_STATUS:false"));
		            }
		         }
		         if (_IS_DELETE != -1)
		         {
