...
using CMCLIS.GATEWAY.WServiceMQDB.LOG.Properties;
        ...
        static void Main(string[] args)
        {
            ...
            using (var channel = connection.CreateModel())
            {
                ...
                consumer.Received += (model, ea) =>
                {
                    ...
                    var message = JsonConvert.DeserializeObject<CVAN_LOG_SENDMQInfo>(json);
                    if (int.Parse(message.Type_Log.ToString()) > 0)
                    {
                        Console.WriteLine("--------------------------------------------------");
                        var result = MQ_PROCESS_DATA.MAIL_INSERT_MSG_MQ(message);
