...
namespace CMCLIS.GATEWAY.WServiceMQD
{
    public class MQ_PROCESS_DATA
    {
        ...
        {            
            ...
            {               
                ...
                if (!Directory.Exists(CMCLIS.GATEWAY.SETTING.Config.PATH_DATA_SAVE_FILE + @"\TIEP_NHAN\" + sender))
                {
                    Directory.CreateDirectory(CMCLIS.GATEWAY.SETTING.Config.PATH_DATA_SAVE_FILE + @"\TIEP_NHAN\" + sender);
                }
                    
                File.WriteAllText(CMCLIS.GATEWAY.SETTING.Config.PATH_DATA_SAVE_FILE + @"\TIEP_NHAN\" + sender + @"\" + MESSAGE_CODE + ".xml", xmlMessage, Encoding.UTF8);
                Console.WriteLine(" [->] {Save file complete...}");
                #endregion
                #region Signature
                Console.WriteLine(" [->] {Loading X509Certificate2...}");
                System.Security.Cryptography.X509Certificates.X509Certificate2 cert = Certificate.GetStoreX509Certificate2BySerial(CMCLIS.GATEWAY.SETTING.Config.CA_TOKENT_SERIAL);
                if (cert != null)
                {
                    Console.WriteLine(" [->] {Loading X509Certificate2 complete...}");
                    if (File.Exists(CMCLIS.GATEWAY.SETTING.Config.PATH_DATA_SAVE_FILE + @"\TIEP_NHAN\" + CMCLIS.GATEWAY.SETTING.Config.MQ_NAME + @"\" + MESSAGE_CODE + ".xml"))
                    {
                        ...
                        {
                            if (!Directory.Exists(CMCLIS.GATEWAY.SETTING.Config.PATH_DATA_SAVE_FILE + @"\DATA_SIGNATURE"))
                                Directory.CreateDirectory(CMCLIS.GATEWAY.SETTING.Config.PATH_DATA_SAVE_FILE + @"\DATA_SIGNATURE");
                            if (!Directory.Exists(CMCLIS.GATEWAY.SETTING.Config.PATH_DATA_SAVE_FILE + @"\DATA_SIGNATURE" + @"\" + DateTime.Now.ToString("yyyy-MM-dd")))
                                Directory.CreateDirectory(CMCLIS.GATEWAY.SETTING.Config.PATH_DATA_SAVE_FILE + @"\DATA_SIGNATURE" + @"\" + DateTime.Now.ToString("yyyy-MM-dd"));
                            string pathSaveFile = CMCLIS.GATEWAY.SETTING.Config.PATH_DATA_SAVE_FILE + @"\DATA_SIGNATURE" + @"\" + DateTime.Now.ToString("yyyy-MM-dd") + @"\" + MESSAGE_CODE + ".xml";
                            ...
                            int totalCount = 0;
                            List<CVAN_DM_MSG_TYPEInfo> typeInfos = DataObjectFactory.GetInstanceCVAN_DM_MSG_TYPE().CVAN_DM_MSG_TYPE_GetAllWithPadding(MESSAGE_TYPE, "", -1,0, 1, 1, ref totalCount);
                            if (typeInfos!=null)
                            {
