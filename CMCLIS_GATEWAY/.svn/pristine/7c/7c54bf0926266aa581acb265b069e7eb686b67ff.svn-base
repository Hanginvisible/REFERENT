                ...
                if (listData != null && listData.Count > 0)
                {
                    ...
                    foreach (var info in listData)
                    {
                        ...
                        try
                        {
                            ...
                            _MSGIInfos= DataObjectFactory.GetInstanceCVAN_MSGI().CVAN_MSGI_GetAllWithPadding("", info.CVAN_MA_GD, MSG_TYPE, completeCode,0, 1, 1, ref totalCount);
                            if (_MSGIInfos!=null)
                            {
                            ...
                            if (result.ResultCode.EqualsValue(CMCLIS.GATEWAY.SETTING.Constant.RETURN_CODE_SUCCESS) && !string.IsNullOrWhiteSpace(result.ReturnValue))
                            {
                                ...
                                if (!string.IsNullOrWhiteSpace(errorStatusCode) || !string.IsNullOrWhiteSpace(successStatusCode))
                                {
                                    ...
                                    _MSGIInfos = DataObjectFactory.GetInstanceCVAN_MSGI().CVAN_MSGI_GetAllWithPadding("", info.CVAN_MA_GD, MSG_TYPE, resultStatusCode,0, 1, 1, ref totalCount);
                                    if (_MSGIInfos != null)
                                    {
                                        _MSGIInfo = _MSGIInfos.FirstOrDefault();
                                    }
